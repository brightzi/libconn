...
int on_status(http_parser* parser, const char *at, size_t length) {
    ...
    HttpParser* hp = (HttpParser*)parser->data;
    HTTP_PARSER_TYPE type = hp->parser_type;
    hp->state = HP_STATUS;
    if (type == HTTP_PARSER_REQUEST) {
        HttpRequest *req = (HttpRequest *)hp->req;
...
int on_body(http_parser* parser, const char *at, size_t length) {
    ...
    HTTP_PARSER_TYPE type = hp->parser_type;
    if (type == HTTP_PARSER_REQUEST) {
        HttpRequest *request = (HttpRequest *)hp->req;
        request->body.append(at, length);
    } else {
        HttpResponse *response = (HttpResponse *)hp->resp;
...
int on_message_complete(http_parser* parser) {
    ...
    hp->invokeHttpCb();
    hp->state = HP_MESSAGE_COMPLETE;
    hp->method = (http_method) parser->method;
    return 0;
}
...
void HttpParser::handle_header() {
    if (header_field.size() != 0) {
        if (parser_type == HTTP_PARSER_REQUEST) {
