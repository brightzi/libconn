...
#define LOOP_ENTRY(p)  container_of(p, struct event_loop_st, node)


static int timers_compare(const heap_node_t lhs, const heap_node_t rhs) {
...
void pipe_read_cb(io_t io, void *buf, int nbytes) {
...
event_loop_t event_loop_init_with_name(const char *name) {
    ...
    loop->pending = NULL;
    loop->npendings = 0;
    loop->timers = create_heap(1000, timers_compare);
...
int event_loop_run(event_loop_t loop) {
    ...
    while (!loop->quit) {
        ...
        if (loop->timers_num > 0) {
            ...
            if (loop->timers && loop->timers->size > 0) {
                int64_t min_timeout = TIMER_ENTRY(loop->timers->array[0])->next_timeout - loop->cur_ms;
        ...
        // printf("event_loop_run, blocktime_ms=%d\n", blocktime_ms);
        update_loop_timer(loop);
        process_pending_event(loop);
    }
    return 0;
...
int event_loop_wakeup(event_loop_t loop) {
    ...
    write(loop->pipefd[1], "c", strlen("c"));
    return 0;
}
...
void process_pending_event(event_loop_t loop) {
    ...
    event_t cur = loop->pending;
    while(cur) {
        event_t next = cur->next_event;
        if (cur->cb) {
            cur->cb(cur);
            cur->pending = 0;
            if (cur->destory) {
...
event_timer_t add_timer(event_loop_t loop, int timeout, timer_cb cb, int repeat) {
    ...
    memset(timer, 0, sizeof(event_timer_st));
...
void reset_timer(event_timer_t timer, int timeout) {
    io_t io = (io_t)timer->privdata;
    if (!timer->destory) {
        heap_pop(io->loop->timers);
    } else {
        event_loop_t loop = io->loop;
        loop->timers_num++;
    }
    timer->next_timeout = io->loop->cur_ms + timeout;
    heap_insert(io->loop->timers, &timer->node);
    event_reset(timer);
    return ;
}
...
io_t create_tcp_client(event_loop_t loop, const char *ip, const char *port, connect_cb connect_cb, close_cb close_cb, void *userdata) {
    ...
    int non_block = 1;
    int client_fd = create_socket(non_block);
...
io_t create_tcp_server(event_loop_t loop, const char *ip, const char *port, accept_cb accept_cb) {
    ...
    int block = 0;
    int server_fd = create_socket(block);
